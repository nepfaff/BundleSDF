...
class NerfRunner:
        ...
        if self.global_step % self.cfg["i_mesh"] == 0 and self.global_step > 0:
            with torch.no_grad():
                ...
                if mesh is not None:
                    ...
                    if self.models["pose_array"] is not None:
                        print("Optimizing poses for offset")
                        ...
                        offset = np.eye(4)
                    print("Converting mesh to real world")
    ...
    def make_key_ray_ids(self):
        ...
        for k in matches_table.keys():
            ...
            def rays_to_vox_ids(rays):
                ...
                rays_near, rays_far, rays_pid, ray_depths_in_out = (
        ...
        if (
            ...
            if invalid_depth_mask.any() and self.cfg["use_octree"]:
                ...
                z_vals_invalid, _ = self.sample_rays_uniform_occupied_voxels(
                    ray_ids=ray_ids[invalid_depth_mask],
    ...
    def raw2outputs(
        ...
        if depth is not None:
            ...
            ) * torch.sigmoid(-sdf_from_depth * self.cfg["sdf_lambda"])
            ...
            cumprod_shifted = torch.cat(
                [
                    ...
                    torch.cumprod(1 - alphas + 1e-10, -1)[:, :-1],
                ],
                -1,
            )
    ...
    def mesh_texture_from_train_images(
        ...
        @mesh: in normalized space
        """
        print("texturing mesh from train images")
    ...
    def mesh_texture_from_train_images_with_interpolation(
        self, mesh, rgbs_raw, train_texture=False, tex_res=1024
    ):
        """
        @rgbs_raw: raw complete image that was trained on, no black holes
        ...
        for i in tqdm(range(len(rgbs_raw)), desc="Projecting train_images"):
            ...
            common.rayColorToTextureImageCUDA(
                ...
                .cuda()
                .long(),  # Mesh faces (triangles) in CUDA tensor
                torch.from_numpy(mesh.vertices)
                .cuda()
                .float(),  # Mesh vertices in CUDA tensor
                torch.from_numpy(locations)
                .cuda()
                .float(),  # 3D locations of hit points
                torch.from_numpy(index_tri)
                .cuda()
                .long(),  # Indices of the triangles hit
                torch.from_numpy(uvs_tex)
                .cuda()
                .float(),  # Precomputed UV coordinates of the mesh
        ...
        min_weight_threshold = (
            1e-5  # A small value to avoid dividing by very small weights
        )
        ...
        print(
            f"Texture image stats before interpolation: min={tex_image.min()}, max={tex_image.max()}, mean={tex_image.mean()}, std={tex_image.std()}"
        )
        ...
        if missing_mask.any():
            ...
            tex_image = self.interpolate_missing_vertices_nn_3d(
                mesh, tex_image, weight_tex_image, uvs_tex, vertices_cuda
            )
        ...
        print(
            f"Texture image stats (final): min={tex_image.min()}, max={tex_image.max()}, mean={tex_image.mean()}, std={tex_image.std()}"
        )
    ...
    def interpolate_missing_vertices_nn_3d(
        self, mesh, tex_image, weight_tex_image, uvs_tex, vertices_cuda, batch_size=5000
    ):
        """
        ...
        uvs = (
            uvs_tex * np.array([tex_image.shape[1] - 1, tex_image.shape[0] - 1])
        ).astype(int)

        # Extract valid pixel locations in the texture image based on weight_tex_image
        valid_mask = weight_tex_image > 0
        valid_uvs = np.argwhere(
            valid_mask.cpu().detach().numpy()
        )  # Get valid UV coordinates in texture space

        # Map valid UVs to corresponding 3D vertices
        valid_vertices = vertices_cuda[
            valid_uvs[:, 1]
        ]  # Use second dimension since uvs_tex corresponds to vertices
        ...
        for i in tqdm(
            range(0, len(missing_uvs), batch_size),
            desc="Interpolating missing vertices",
        ):
            ...
            batch_missing_uvs_3d = vertices_cuda[
                np.argmin(
                    np.linalg.norm(
                        uvs[:, None, :] - batch_missing_uvs[None, :, :], axis=2
                    ),
                    axis=0,
                )
            ]
            ...
            nearest_colors = tex_image[
                valid_uvs[nearest_indices][:, 0], valid_uvs[nearest_indices][:, 1]
            ]
    ...
    def interpolate_missing_vertices_gaussina(self, tex_image, missing_mask):
        ...
        gaussian_kernel_1d = torch.exp(
            -((grid - kernel_size // 2) ** 2) / (2 * sigma**2)
        )
        gaussian_kernel_1d = gaussian_kernel_1d / gaussian_kernel_1d.sum()

        # Create a 2D separable Gaussian kernel for 3 channels (RGB)
        gaussian_kernel_2d = gaussian_kernel_1d.view(
            1, 1, -1, 1
        ) * gaussian_kernel_1d.view(1, 1, 1, -1)
        gaussian_kernel_2d = gaussian_kernel_2d.repeat(
            3, 1, 1, 1
        ...
        valid_tex_image_padded = torch.nn.functional.pad(
            valid_tex_image.permute(2, 0, 1).unsqueeze(0),
            (kernel_size // 2, kernel_size // 2, kernel_size // 2, kernel_size // 2),
            mode="reflect",
        )
        tex_image_smoothed = (
            conv2d(valid_tex_image_padded, gaussian_kernel_2d, groups=3)
            .squeeze()
            .permute(1, 2, 0)
        )
    ...
    def mesh_texture_from_nerf(self, mesh, tex_res=1024):
        ...
        tex_image = torch.zeros(
            (tex_res, tex_res, 3), dtype=torch.float32, device=self.device
        )
        weight_tex_image = torch.zeros(
            (tex_res, tex_res), dtype=torch.float32, device=self.device
        )
        ...
        uvs_tex = (
            mesh.visual.uv * np.array([W_tex - 1, H_tex - 1]).reshape(1, 2)
        ).astype(
            np.float32
        )  # (n_V, 2)
        ...
        for i in range(0, faces_cuda.shape[0], ray_chunk_size):
            ...
            face_vertices = vertices_cuda[faces_cuda[i : i + ray_chunk_size]].reshape(
                -1, 3, 3
            )

            # Compute barycentric coordinates and map to UV coordinates
            uvs = self.barycentric_interpolation(
                face_vertices, uvs_tex[i : i + ray_chunk_size]
            )

            # Query the NeRF model using the interpolated points
            inputs = face_vertices.reshape(-1, 1, 3)
            viewdirs = torch.zeros(
                (inputs.shape[0], 3), dtype=torch.float32, device=self.device
            )  # No viewdirs used
            frame_ids = torch.zeros(
            ...
            with torch.no_grad():
                outputs_local, _, _ = self.run_network(
                    inputs=inputs, viewdirs=viewdirs, frame_ids=frame_ids, tf=tf
                )

            # Retrieve the RGB color values
            ray_colors = (
                torch.sigmoid(outputs_local[..., :3]).reshape(-1, 3).cpu().numpy()
            )
    ...
    def barycentric_interpolation(self, face_vertices, uvs_tex):
        ...
        uvs = (
            (
                torch.cross(v1 - v0, v2 - v0, dim=-1).norm(dim=-1, keepdim=True)
                / area_total
            )
            * uvs_tex[:, 0:1]
            + (
                torch.cross(v2 - v1, v0 - v1, dim=-1).norm(dim=-1, keepdim=True)
                / area_total
            )
            * uvs_tex[:, 1:2]
            + (
                torch.cross(v0 - v2, v1 - v2, dim=-1).norm(dim=-1, keepdim=True)
                / area_total
            )
            * uvs_tex[:, 2:3]
        )
