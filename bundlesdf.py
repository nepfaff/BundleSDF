...
if not os.path.exists(bundle_track_build):
    raise RuntimeError(
        f"BundleTrack build directory not found at {bundle_track_build}. Please run setup_without_venv.bash first."
    )
sys.path.append(bundle_track_build)
...
class BundleSdf:
    def __init__(
        self,
        cfg_track_dir=f"{code_dir_bundlesdf}/BundleTrack/config_ho3d.yml",
        cfg_nerf_dir=f"{code_dir_bundlesdf}/config.yml",
    ...
    def run_global_nerf(
        ...
        if use_all_frames:
            print(
                f"Using all frames in {self.debug_dir}/color_segmented for global nerf"
            )
            if not os.path.exists(f"{self.debug_dir}/color_segmented"):
                raise ValueError(
                    f"Directory {self.debug_dir}/color_segmented not found"
                )
            ...
            keyframes = yaml.load(
                open(f"{self.debug_dir}/{last_stamp}/keyframes.yml", "r")
            )
            ...
            if len(keyframes) > self.cfg_nerf["n_train_image"]:
                keys = [keys[0]] + list(
                    np.random.choice(
                        keys, self.cfg_nerf["n_train_image"], replace=False
                    )
                )
                keys = list(set(keys))
        ...
        sc_factor, translation, _, pcd_normalized = compute_scene_bounds(
            None,
            glcam_in_obs,
            self.K,
            use_mask=True,
        ...
        if get_texture:
            if interpolate_missing_vertices:
                mesh = nerf.mesh_texture_from_train_images_with_interpolation(
...
if __name__ == "__main__":
    ...
    cfg_nerf = yaml.load(
        open(f"{code_dir_bundlesdf}/BundleTrack/config_ho3d.yml", "r")
    )
    cfg_nerf["data_dir"] = (
