...
def run_one_video(
    ...
    cfg_bundletrack["keyframe"][
        "min_rot"
    ] = 5  # Minimum rotation in degrees to consider a frame as keyframe
...
def run_one_video_global_nerf(
    out_folder="/home/bowen/debug/bundlesdf_scan_coffee_415", use_dino_frames=False
):
    set_seed(0)
    ...
    cfg_nerf["mesh_resolution"] = (
        0.002  # Decrease for smaller marching cubes voxel size
    )
    cfg_nerf["n_train_image"] = 1200 # Maximum number of images for global nerf training
    cfg_nerf["fs_sdf"] = 0.1
    ...
    tracker.run_global_nerf(
        reader=reader, get_texture=True, tex_res=2048, use_dino_frames=use_dino_frames
    )
    tracker.on_finish()
...
if __name__ == "__main__":
    ...
    if args.mode == "run_video":
        ...
        run_one_video_global_nerf(out_folder=args.out_folder, use_dino_frames=True)
